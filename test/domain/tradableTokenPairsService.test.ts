...
import { TradableTokenPairsService } from "../../src/domain/index";
import { amadeusServiceStubFactory, tickerServiceStubFactory, tokenServiceStubFactory } from "../stubs";
...
describe("TradableTokenPairsService", () => {
    const iocContainer = createContainer(true, tickerServiceStubFactory, tokenServiceStubFactory, amadeusServiceStubFactory, (c: Container) => {
        c.bind<TokenPairsService>(TYPES.TokenPairsService).to(TradableTokenPairsService);
    });
            ...
            context("as a number greater than zero", () => {
                ...
                context("and it is the second page", () => {
                    it("should return only items from that page", async () => {
                        ...
                        expect(returned).to.be.an("array").that.has.lengthOf(1);
                        expect(returned.findIndex((pair) => pair.tokenA.address === tokenAddress2 && pair.tokenB.address === tokenAddress3)).to.be.equal(0);
                    });
                });
            });
        });
        context("When perPage is informed", () => {
            ...
            context("as a number greater than zero", () => {
                context("and there is more than one page", () => {
                    context("and the page is 1", () => {
                        it("should return the exact number of pairs of perPage parameter", async () => {
                            const returned = await iocContainer.get<TokenPairsService>(TYPES.TokenPairsService).listPairs(undefined, undefined, 1, 3);
                            expect(returned).to.be.an("array").that.has.lengthOf(3);
                        });
                    });
                ...
                context("and there is only one page", () => {
                    it("should return only the remaining pairs", async () => {
                        const returned = await iocContainer.get<TokenPairsService>(TYPES.TokenPairsService).listPairs(undefined, undefined, 1, 6);
                        expect(returned).to.be.an("array").that.has.lengthOf(3);
                    });
                });
            });
